...
- **Entry**: `main.py` – orchestrates the pipeline (preferences → fetch → normalize → extract → summarize → optional export). Add new pipeline steps here or call new modules from here.
...
- **Data**: `src/data_source.py` – JSearch API vs mock file selection, raw response save to `debug/api-response/<timestamp>_response.json`. Add new job sources (other APIs, scrapers) here; keep the same contract: return a raw response dict and let `normalize`/`extract` stay source-agnostic.
...
| New output format (e.g. Markdown export) | `src/summary.py` and `main.py` (export prompt / CLI) |
